...
class WorldTileWorker {
    ...
    getNeighbours(faceIndexHigh, faceIndexLow, recursion) {
        const neighbours = [];
        const thisFace = this.terrainGenerator.getFaces(faceIndexHigh, faceIndexLow, recursion, recursion);
    ...
    generateTerrain(faceIndexHigh, faceIndexLow, recursion, requiredDepth, skirt) {
        /* console.log(faceIndexHigh,
                    faceIndexLow,
                    recursion,
                    requiredDepth);*/
        const faces = this.terrainGenerator.getFaces(faceIndexHigh, faceIndexLow, recursion, requiredDepth);
    ...
    getSurfaceUnderPoint(point, recursion) {
        point = new THREE.Vector3(point.x, point.y, point.z);
        const direction = new THREE.Vector3();
        direction.copy(point);
        direction.normalize();
        direction.negate();
        const face = this.terrainGenerator.rayCrossesFace(point.toArray(), direction.toArray(), recursion);
        if (face) {
            const surfacePoint = new THREE.Vector3((face.points[0][0] + face.points[1][0] + face.points[2][0]) / 3, (face.points[0][1] + face.points[1][1] + face.points[2][1]) / 3, (face.points[0][2] + face.points[1][2] + face.points[2][2]) / 3);
    ...
    getFaceUnderMouse(mouseRay, recursion) {
        ...
        const face = this.terrainGenerator.rayCrossesFace(mouseRay.origin, mouseRay.direction, recursion);
        if (face) {
            const surfacePoint0 = {
...
onmessage = (e) => {
    ...
    switch (e.data[0]) {
        case "rayCrossesFace":
            break;
        case "getNeighbours":
            const neighbours = worldTileWorker.getNeighbours(e.data[1], e.data[2], e.data[3]);
            reply.push(neighbours);
            socket.postMessage(reply);
            break;
        case "generateTerrain":
            const geometry = worldTileWorker.generateTerrain(e.data[1], e.data[2], e.data[3], e.data[4], e.data[5]);
            socket.postMessage(reply.concat(geometry), geometry);
            break;
        case "getSurfaceUnderPoint":
            const point = worldTileWorker.getSurfaceUnderPoint(e.data[2], e.data[3]);
            reply.push(point);
            socket.postMessage(reply);
            break;
